...
if(localStorage.getItem('yearlyTasks') !== null){
    const yearlyTasks = JSON.parse(localStorage.getItem('yearlyTasks'))
    Array.from(yearlyTasks).forEach((task) => {
...
submitTask.addEventListener('submit', function(e){
    ...
    // create task element
    CreateTaskElement(taskInput, year, noteInput)

    // add task details to local storage
    if(localStorage.getItem('yearlyTasks') === null){
            localStorage.setItem('yearlyTasks',JSON.stringify([{taskTitle:taskInput,
                                                               time:year,
                                                               note:noteInput}]))
...
listContainer.addEventListener('click', function(e){
    if(e.target.className == 'delete'){
        ...
        let yearlyTasks = JSON.parse(localStorage.getItem('yearlyTasks'))
        yearlyTasks = Array.from(yearlyTasks).filter((task)=> task.taskTitle !== currentTaskTilte)
        localStorage.setItem('yearlyTasks',JSON.stringify(yearlyTasks))    
    }
})
